...
- **Pi Zero W Rev 1.1 — on the shelf, unassigned.** Single-core ARMv6 (BCM2835), **512 MB**, 32-bit only, **no ADC**, USB OTG host. It is the original Zero W, **not a Zero 2 W** — the Zero 2 W was an open question in the first roadmap and was never resolved by buying one (`git show c57a002`). Dropped as the kiosk host: too slow for Chromium, and the original roadmap's own rule was "never run Node/servers on the Zero W." Fire HD 8 replaced that experiment.
  ...
  - **Keep/sell call 2026-08-13: keep, conditionally.** Money is not the argument — it resells for **$10–20**, not worth the listing and the meetup. The real cost is that an unassigned spare stays a question that gets re-litigated (this was at least the second pass). **So: keep it assigned to the watchdog; if that is not built by 2026-12-01, sell it.** One real keep-argument beyond that — it is the only board in the house that could take a **USB cellular dongle** (needs a Linux USB host, which an ESP8266 cannot be), which is the only path to covering the outage case. ~$25 + ~$5/mo SIM, not recommended now. **Sell instead if** the watchdog isn't wanted, or if the gap gets solved properly with an off-premises uptime service pinging a Tailscale endpoint — no hardware, and it covers the outage case the Zero W cannot
