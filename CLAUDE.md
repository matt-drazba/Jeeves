...
- **Alerting system live** — `homeassistant/packages/jeeves_alerts.yaml`, deployed via `git pull` on the Pi. Two docs: `docs/alerting_levels.md` is the spec (why the levels are what they are), `docs/alerting_runbook.md` is the operator manual (what to do when one fires, acknowledging, deploy, troubleshooting)
  ...
  - **`out_temp_offset_f` zeroed 2026-08-08** (was 0.3). 102 pump-running/heat-off samples put the raw probes a median 0.00°F apart — they agree to within one 12-bit LSB (0.1125°F). The old 0.3 came from a single 2026-08-05 spot reading that no longer reproduces. **This had been silently disarming the HX L3**: that alert fires on ΔT ≤ 0, and a permanent +0.3 meant a dead heat exchanger still read positive. Watch it now that it is genuinely armed for the first time — if a real low-stage run quantizes to 0.00 or −0.11 for 20 min it will fire, and the fix would be a threshold clearly below zero rather than at it. Re-check the offset over a wider water-temp range; if the gap tracks temperature, switch to `calibrate_linear`
  ...
  - **`verify-alerts.py` gap:** it scans only `jeeves_alerts.yaml`, so the `ALERT_REGISTRY` in `jeeves/server.js` (7 flag entities + 6 detector sensors) is unchecked — the exact silent-failure mode the script exists to catch. Extending its file list is ~3 lines and would cover every HA entity Jeeves references. Not done
...
- **Fire HD 8 kiosk live (2026-08-08)** — Fully Kiosk Browser on the wall tablet, pointed at `http://192.168.0.189:3000`. Setup guide, settings table, and gotchas in `docs/fire_tablet_kiosk.md`
  - **Kiosk Mode is a Fully PLUS feature**, as are Remote Admin and motion detection. PLUS is unlimited free to try, so the setup can be proven before paying — **but unlicensed Fully shows the kiosk PIN on screen as a hint** (observed 2026-08-08), so anyone can read it and exit. The license is what makes the lockdown real, not a nag removal
...
- **Multiport valve position sensing (the right replacement for maintenance mode):** let HA know when the Tagelus TA100D handle is *not* in Filter — Backwash, Rinse, Waste, Recirculate, Closed. Idea is a tag/trigger mapped to the handle position. This is what maintenance mode was reaching for and got wrong: valve position is **self-clearing** (turn the handle back and monitoring resumes), whereas a mute toggle waits to be remembered and silently leaves the pool unmonitored. With it, servicing suppression becomes a fact about the equipment instead of a promise from the operator, and it additionally makes backwash duration loggable for the copper model (`docs/pool_data_addendum.md`) and enables a real "handle left in Backwash" alert — a genuine drain-the-pool hazard. Hardware TBD: reed/hall switches per detent, a rotary encoder, or an NFC tag tapped on the way past.
