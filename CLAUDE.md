...
- **Garage door live (2026-08-10)** — ratgdo32 at `192.168.0.230`, ESPHome firmware, **Security+ 1.0 with ratgdo emulating the wall panel**. Confirmed wired and talking by watching the device's own event stream (polling bytes `38/39/3A` on the bus, `Light state` + `Lock state` coming back). Full doc: `docs/garage_door.md`; automations in `homeassistant/packages/jeeves_garage.yaml`
  - **Two entities on that device are traps — do not build on either.** `binary_sensor Motion` exists but will never fire: Security+ 1.0 motion comes from a motion-equipped wall panel, and ratgdo is *replacing* the panel, not listening to one. `binary_sensor Obstruction` reads `OFF` and its wire may never have been landed — an always-clear obstruction sensor is a **fake safety condition**, worse than none because it looks like a real check. Everything is built on `cover.garage_door` alone, and an obstruction is detected the way it actually manifests: the door starts closing, the opener's own photo-eyes reverse it, and it never reaches `closed`
