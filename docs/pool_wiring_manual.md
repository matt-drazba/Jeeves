...
- The IntelliComm inputs are **voltage-driven (9–24V AC/DC), not dry-contact** — a relay must *switch a live leg onto* the input, exactly as the FPH signal does on input 4, not close a bare contact across the input terminals.
- **Source it from the 12V adapter already in the chimney box** — the one feeding the IntelliComm and the ESP's buck converter. 12V is inside the 9–24V input spec. *This line previously read "switch 24VAC onto the input … from the same transformer pair, not a new supply," which disagreed with the note in `esphome/pool-pad.yaml`. Reconciled 2026-08-13:* both are electrically legal, but the only 24VAC present is the FPH trio leg — the **flow-switch-gated safety circuit**. Do not hang a discretionary HA pump command off the safety chain when an independent supply is already sitting in the same box.
...
- **GPIO13 may not still be free by then.** [pool_data_addendum.md](pool_data_addendum.md) claims GPIO13 *and* GPIO4 as the I2C bus for the planned ADS1115 filter-pressure ADC. If that ships first, use **GPIO16 (D0)** for this relay — output-capable and idles LOW, exactly the polarity wanted. It is missing from the 4.2 pin map only because that map covers *inputs*; GPIO16 has no interrupt and no pull-up, and a relay drive needs neither.
            ...
            └─ ~0V
               ...
               └─ Switch CLOSED (no voltage across it)
                   → Signal isn't arriving from the FPH controller at all.
                     ...
                     and confirm the IntelliComm control wires are landed per
                     the FPH manual. Inside the FPH — installer/HotSpot
                     territory.
...
|---|---|---|
| **FPH LCD says "Heating" but the condenser fan keeps spinning** | Diversion never engaged — the terminal-4 branch. Meter the 90340 coil before condemning a part | 6.1, 6.5 |
...
**⚠ Two failures look alike; the coil tells them apart.** Flow switch **closed** (~0 V across it) but the 90340 coil also reading **~0 V** means the terminal-4 signal never arrived — a wiring or controller-output fault. A genuinely failed 90340 reads the opposite: **~24VAC across the coil** with the contacts not moving. The fix is completely different, so meter the coil before ordering a relay.
...
| 2026-08-08 | **Reverted a same-day addition documenting the IntelliComm yellow/green conductor rule.** It is standard FPH content, already covered by the installer manual, so it does not belong here — this document exists for what *differs* from that manual. 6.1 keeps a one-line pointer to land the IntelliComm wires per the FPH manual; 6.4's metering note was generalised to "signal never arrived vs. relay failed," which is useful regardless of cause. |
